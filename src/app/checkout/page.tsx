...
import { readCart, itemUnitPrice, clearCart } from "@/lib/cart";
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    // Read the stored cart and normalize it to the lightweight CartItem used by orders.
    // (Avoid relying on a specific named export to prevent deployment/type issues.)
    const raw = readCart();
    const normalized: CartItem[] = raw.map((i: any) => ({
      productId: String(i.id),
      title: String(i.title ?? ""),
      modelNumber: Number(i.modelNumber ?? 0),
      price: Number(itemUnitPrice(i)),
      qty: Number(i.qty ?? 1),
      image: i.imageUrl,
    }));
    setItems(normalized);
